        ...
        private void dgvSanPham_Click(object sender, EventArgs e)
        {
            try
            {
                if (dgvSanPham.SelectedRows.Count == 1)
                {
                    ...
                    //if (cboLoai.Items.Contains(dr.Cells["Tên Loại Sản Phẩm"].Value.ToString().Trim())) ;
                    MessageBox.Show(dr.Cells["Tên Loại Sản Phẩm"].Value.ToString().Trim());
                    cboNCC.SelectedValue = NCCDL.GetInstance.GetMaNSX_TenNSX(dr.Cells["Tên Nhà Sản Xuất"].Value.ToString().Trim());

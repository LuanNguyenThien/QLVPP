    ...
    public partial class frmLoaiSanPham : Form
    {
        ...
        private void dgvLoaiSanPham_Click(object sender, EventArgs e)
        {
            if (dgvLoaiSanPham.SelectedRows.Count == 1)
            {
                ...
                maloaisp = dr.Cells["Mã Loại Sản Phẩm"].Value.ToString().Trim();
                txtTen.Text = dr.Cells["Tên Loại Sản Phẩm"].Value.ToString().Trim();
            }
        }
        ...
        private void btnCapNhat_Click(object sender, EventArgs e)
        {
            ...
            lsp.TenLoaiSP = txtTen.Text;
            if (LoaiSanPhamBL.GetInstance.CapNhatLoaiSanPham(lsp))
            {
                picThanhCong.Visible = true;
                timer.Start();
                LoadDataGridView();

...
namespace QuanLyCuaHangBanDoChoi.UserControls
{
    public partial class ucQuanLySanPham : UserControl
    {
        ...
        private void dgvSanPham_Click(object sender, EventArgs e)
        {
            try
            {
                if (dgvSanPham.SelectedRows.Count == 1)
                {
                    ...
                    cboLoai.SelectedValue = LoaiSanPhamDL.GetInstance.GetMaLSP_TenLSP(dr.Cells["Tên Loại Sản Phẩm"].Value.ToString().Trim());
                    cboNCC.SelectedValue = NSXDL.GetInstance.GetMaNSX_TenNSX(dr.Cells["Tên Nhà Sản Xuất"].Value.ToString().Trim());
                    ...
                    if (dr.Cells["Tình Trạng"].Value.ToString() == "Ngưng bán")
                    {
            ...
            if (CheckControls())
            {
                if (SanPhamBL.GetInstance.NgungKinhDoanhSanPham(masp.ToString()))
                {
                    ...
                    if (btnThemSP.Enabled == true)
                        ...
                        this.Alert("Đã khôi phục kinh doanh...", frmPopupNotification.enmType.Success);
                }
            }
            else
            {
        ...
        {
            dgvSanPham.DataSource = SanPhamBL.GetInstance.GetDanhSachSanPhamTheoBoLoc(txtTenSP.Text.Trim(), cboLocLoaiSP.Text.Trim(), cboLocNCC.Text.Trim());
        ...
        {
            if (txtLoiNhuan.BackColor == Color.OrangeRed)
            {
        ...
        {
            if (txtGiaNhap.BackColor == Color.OrangeRed)
            {

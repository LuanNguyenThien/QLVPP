    ...
    public partial class ucNhapSanPham : UserControl
    {
        ...
        private void btnTaoPhieu_Click(object sender, EventArgs e)
        {
            ...
            if (frm.DialogResult == DialogResult.OK)
            {
                ...
                pnDTO.MaNV = frmDangNhap.MaNV;
                pnDTO.MaNSX = frm.MANSX;
                mansx = pnDTO.MaNSX;
                tennsx = frm.TENNSX;
                pnDTO.NgayNhap = DateTime.Now;
                pnDTO.TrangThai = "Chưa nhập";
                dgvSanPham.DataSource = SanPhamBL.GetInstance.GetDanhSachSanPhamTheoNSX(tennsx);
            ...
            if (mapn != "")
            {
                try
                {
                    if (dgvSanPham.SelectedRows.Count == 1)
                    {
                        ...
                        txtMaSP.Text = dr.Cells["Mã Sản Phẩm"].Value.ToString().Trim();
                        txtTenSP.Text = dr.Cells["Tên Sản Phẩm"].Value.ToString().Trim();
                        txtDonGiaNhap.Text = dr.Cells["Giá Nhập"].Value.ToString().Trim();
                    }
                }
                catch (Exception)
                {
        ...
        private void btnThem_Click(object sender, EventArgs e)
        {
            ...
            dgvCTPN.Rows.Insert(dgvCTPN.Rows.Count, txtMaSP.Text, mapn.ToString(), frmDangNhap.MaNV, txtTenSP.Text, txtDonGiaNhap.Text, txtSoLuong.Text);
            txtMaSP.Clear();
            txtTenSP.Clear();
            txtSoLuong.Clear();
            txtDonGiaNhap.Clear();
        ...
        private void dgvPhieuNhap_Click(object sender, EventArgs e)
        {
            if (dgvPhieuNhap.SelectedRows.Count > 0)
            {
                ...
                maphieu = dr.Cells["Mã Đơn Nhập Hàng"].Value.ToString().Trim();
        ...
        {
            bool result = false;
            if (btnDaNhap.BackColor == Color.FromArgb(17, 145, 249) && maphieu != "")
            {
        ...
        {
            if (btnXoa.BackColor == Color.FromArgb(17, 145, 249) && maphieu != "")
            {

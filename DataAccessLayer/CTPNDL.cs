...
namespace DataAccessLayer
{
    public class CTPNDL
    {
        ...
        public bool ThemCTPN(DataTable dt, string MAPN)
        {
            try
            {
                int rows = 0;
                ...
                {
                    string sql = "INSERT INTO NhapHang VALUES('" + MAPN + "','" + dt.Rows[i][0].ToString() + "','" + dt.Rows[i][5].ToString() + "','" + dt.Rows[i][4].ToString() + "')";
        ...
        public DataTable GetDanhSachPhieuNhap(string MAPN)
        {
            try
            {
                string sql = "SELECT MaSP, SoLuong FROM NhapHang WHERE MaDNH= '" + MAPN + "'";

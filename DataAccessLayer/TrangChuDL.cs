...
namespace DataAccessLayer
{
    public class TrangChuDL
    {
        ...
        public int GetTongSanPhamDaBan()
        {
            try
            {
                string sql = "SELECT dbo.TotalProductQuantity()";
        ...
        public double GetTongDoanhThu()
        {
            try
            {
                string sql = "SELECT dbo.GetTotalPaidRevenue()";
                DataTable dt = new DataTable();
                dt = DataProvider.GetTable(sql);
        ...
        public int GetTongKhachHang()
        {
            try
            {
                string sql = "SELECT dbo.GetKhachHangCount()";
                DataTable dt = new DataTable();
                dt = DataProvider.GetTable(sql);
        ...
        public List<SanPhamDTO> GetTop10SPTheoSoLuongHomNay()
        {
            try
            {
                string sql = "SELECT * FROM dbo.RevenueByDay()";
                DataTable dt = new DataTable();
        ...
        public List<SanPhamNew> GetTop10SPTheoSoLuongThangTruoc()
        {
            try
            {
                string sql = "select * from fn_Top10SanPhamBanChay()";
                DataTable dt = new DataTable();
                List<SanPhamNew> lstSP = new List<SanPhamNew>();
                dt = DataProvider.GetTable(sql);
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    SanPhamNew spDTO = new SanPhamNew();
                    spDTO.MaSP = dt.Rows[i][0].ToString();
                    spDTO.TenSP = dt.Rows[i][1].ToString();
                    spDTO.SoLuong = int.Parse(dt.Rows[i][2].ToString());
        ...
        public List<DoanhThuDTO> GetDoanhThuThangTruoc()
        {
            try
            {
                string sql = "SELECT * FROM RevenueByDay()";
                DataTable dt = new DataTable();
                List<DoanhThuDTO> lstdtDTO = new List<DoanhThuDTO>();
                dt = DataProvider.GetTable(sql);
                for (int i = 0; i < dt.Rows.Count; i++)
